#!/usr/bin/env python3
"""
Comprehensive Test Script for AgriUnity System (Fixed)
...
def test_hub_optimizer():
    ...
    try:
        ...
        farmers_data = [
            {
                'username': 'farmer_hyderabad_test',
                'name': 'Ramesh Kumar',
                'latitude': 17.3850,
                'longitude': 78.4867,
                'pincode': '500001',
            ...
            {
                'username': 'farmer_vijayawada_test',
                'name': 'Suresh Reddy',
                'latitude': 16.5062,
                'longitude': 80.6480,
                'pincode': '520001',
        ...
        # Create buyer in central location with unique GST
        buyer_data = {
            'username': 'buyer_central_test',
            'name': 'Central Foods Ltd',
            'latitude': 16.9456,
            'longitude': 79.5674,
            'pincode': '500001',
            'business_name': 'Central Foods Ltd',
            'gst_number': 'GST987654321'  # Unique GST number
        }
        ...
        deal_group, created = DealGroup.objects.get_or_create(
            group_id='COMPREHENSIVE-TEST-FIXED-POTATO-FAQ-202412010000',
            defaults={
...
def test_market_analyzer():
    ...
    try:
        ...
        # Test market insights using actual methods
        print("\n🔍 Testing MarketAnalyzer.get_market_data")
        try:
            market_analyzer = MarketAnalyzer()
            
            # Test with actual crop data
            ...
            if test_crop:
                ...
                if market_data:
                    print("✅ MarketAnalyzer.get_market_data executed successfully")
                    print(f"   Result type: {type(market_data)}")
                    print(f"   Crop: {market_data.get('crop_name', 'Unknown')}")
                    print(f"   District: {market_data.get('district', 'Unknown')}")
                    print(f"   Grade: {market_data.get('grade', 'Unknown')}")
                    print(f"   Data points: {market_data.get('data_points', 0)}")
                    
                    # Check price analysis
                    price_analysis = market_data.get('price_analysis', {})
                    if price_analysis:
                        print(f"   Current price: ₹{price_analysis.get('current_price_per_kg', 'Unknown')}")
                        print(f"   Price trend: {price_analysis.get('price_trend', 'Unknown')}")
                    
                    # Check market insights
                    market_insights = market_data.get('market_insights', {})
                    if market_insights:
                        print(f"   Market insights available: {len(market_insights)} items")
                else:
            ...
            print(f"❌ MarketAnalyzer.get_market_data failed: {e}")
            import traceback
            traceback.print_exc()
        
        # Test available crops and districts
        print("\n🔍 Testing MarketAnalyzer.get_available_crops")
        try:
            available_crops = market_analyzer.get_available_crops()
            if available_crops:
                print(f"✅ Available crops: {len(available_crops)} found")
                print(f"   Sample crops: {available_crops[:5]}")
            else:
                print("⚠️ No crops available")
        except Exception as e:
            print(f"❌ get_available_crops failed: {e}")
        
        print("\n🔍 Testing MarketAnalyzer.get_available_districts")
        try:
            available_districts = market_analyzer.get_available_districts()
            if available_districts:
                print(f"✅ Available districts: {len(available_districts)} found")
                print(f"   Sample districts: {available_districts[:5]}")
            else:
                print("⚠️ No districts available")
        except Exception as e:
            print(f"❌ get_available_districts failed: {e}")
...
def test_ai_agent():
    ...
    try:
        ...
        deal_group = DealGroup.objects.filter(group_id__contains='COMPREHENSIVE-TEST-FIXED').first()
...
def main():
    ...
    print("🚀 Starting Comprehensive AgriUnity System Test (Fixed)")
    print("=" * 80)
